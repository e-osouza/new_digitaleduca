...
import { mediaDoVideo, minhaAvaliacao, obterConteudo } from "@/lib/queries";
...
export default async function Assistir({
  ...
  const [{ id }, { aula }] = await Promise.all([params, searchParams]);
  ...
  try {
    ...
    if (erro instanceof ApiError) {
      ...
      if (erro.semAssinatura) {
        return <SemAcesso voltarPara={`/conteudo/${conteudoId}`} />;
      }
    }
    throw erro;
  }
  ...
  const atual =
    aulas.find((v) => v.id === selecionadaId) ?? aulas[0] ?? null;
  ...
  // O progresso vem embutido no detalhe, filtrado para o usuário logado.
  const progresso = atual.ProgressoVideo?.[0] ?? null;
  ...
  const [media, minhaNota] = await Promise.all([
        ...
        */}
        <div className="bg-fundo sticky top-0 z-30 sm:static">
          <Player
            ...
            vimeoId={vimeoId}
            videoId={atual.id}
            segundosIniciais={progresso?.segundos ?? 0}
            titulo={atual.titulo}
          />
        </div>
